...
(function () {
  ...
  DIAGRAMS["triangle-center-angles"] = [(() => {
    const A2 = [200, 62], B2 = [78, 268], C2 = [352, 268];
    const O = circumcenterOf(A2, B2, C2), R = dist(O, A2);
    const I = incenterOf(A2, B2, C2);
    const cen2 = centroidOf(A2, B2, C2);
    const vl = (p, s) => txt(add(away(p, cen2, 16), [0, 4.5]), s, DIM, 13);
    return wrap(440, 375, [
      ...
      vl(A2, "A"), vl(B2, "B"), vl(C2, "C"),
      angleArc(A2, B2, C2, 26, GLD), txt(add(A2, [0, 34]), "A", GLD, 12.5),
      seg(O, B2, ACC, 1.6), seg(O, C2, ACC, 1.6), dot(O, ACC, 4.5),
      txt(add(O, [0, -12]), "O", ACC, 12.5),
      angleArc(O, B2, C2, 24, ACC), txt(add(O, [0, 30]), "2A", ACC, 12),
      seg(I, B2, GRN, 1.6), seg(I, C2, GRN, 1.6), dot(I, GRN, 4.5),
      txt(add(I, [16, 2]), "I", GRN, 12.5),
      angleArc(I, C2, B2, 22, GRN), txt(add(I, [0, 26]), "90°+A/2", GRN, 11),
      cap(440, 375, "from side BC: the circumcenter O subtends 2A (central angle), the incenter I subtends 90° + A/2")
    ]);
  })()];
